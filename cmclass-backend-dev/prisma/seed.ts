...
const img = (seed: string, w = 1200, h = 1600) =>
  `https://picsum.photos/seed/${encodeURIComponent(seed)}/${w}/${h}`;

const placehold = (bg: string, fg: string, text: string, w = 240, h = 80) =>
  `https://placehold.co/${w}x${h}/${bg}/${fg}?text=${encodeURIComponent(text)}`;

const baseSizes = ['S', 'M', 'L', 'XL'];

const makeColors = (seed: string) => [
...
async function main() {
  ...
  await prisma.brand.upsert({
    ...
    update: {
      ...
      description:
        "Maison de couture nee a Goma, alliant artisanat d'exception, minimalisme et heritage culturel.",
      ...
      servicesHeaderDescription:
        'Des services sur mesure pour accompagner votre style, du conseil a la confection.',
    },
    create: {
      id: 1,
      name: 'CM CLASS',
  ...
  const aboutData = {
    ...
    ctaDescription:
      "Prenez rendez-vous pour decouvrir nos creations ou discuter d'un projet sur mesure.",
  ...
  const productSeeds = [
    {
      ...
      longDescription:
        'Une chemise signature en coton peigne, pensee pour une silhouette nette et contemporaine.',
    ...
    },
    {
      ...
      longDescription:
        'Popeline douce et finitions soignees pour une elegance facile au quotidien.',
    ...
    },
    {
      ...
      longDescription:
        'Une chemise legere pensee pour la saison chaude, avec un tombe fluide.',
  ...
  if (campaignCount === 0) {
    ...
    await prisma.campaign.create({
      data: {
        ...
        selectedProductIds: campaignOneProducts.map((p: any) => p.id),
      },
    });
    ...
    await prisma.campaign.create({
      data: {
        ...
        selectedProductIds: campaignTwoProducts.map((p: any) => p.id),
      },
    });
  }
  ...
  if (products.length > 0) {
    ...
    const wishlistRows: { userId: number; productId: number }[] = [];
    const cartRows: { userId: number; productId: number; quantity: number; size: string; color: string }[] = [];
