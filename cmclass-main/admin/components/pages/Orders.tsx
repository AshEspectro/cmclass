...
import { Search, Download, MessageCircle, AlertTriangle, Phone, Mail, Clock } from 'lucide-react';
                ...
                >
                  ...
                  <td className="px-4 py-3 text-gray-900 font-medium">
                    ...
                    {(order.phone || order.email) && (
                      <div className="flex items-center gap-3 mt-1 text-xs font-normal">
                        {order.phone && (
                          <a href={`tel:${order.phone}`} className="flex items-center gap-1 text-[#007B8A] hover:underline" onClick={e => e.stopPropagation()}>
                            <Phone size={12} /> {order.phone}
                          </a>
                        )}
                        {order.email && (
                          <a href={`mailto:${order.email}`} className="flex items-center gap-1 text-gray-500 hover:underline" onClick={e => e.stopPropagation()}>
                            <Mail size={12} /> Email
                          </a>
                        )}
                      </div>
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex flex-wrap gap-1 justify-center">
                      {(order.badges || []).includes('whatsapp_sent') && <div title="WhatsApp Envoyé"><MessageCircle size={14} className="text-green-500" /></div>}
                      {(order.badges || []).includes('whatsapp_failed') && <div title="Échec WhatsApp"><AlertTriangle size={14} className="text-red-500" /></div>}
                      {((order.badges || []).includes('reservation_expiring_soon') || (order.badges || []).includes('pickup_expiring_soon')) && (
                        <div title="Expire bientôt"><Clock size={14} className="text-orange-500" /></div>
                      )}
                    </div>
                  </td>
